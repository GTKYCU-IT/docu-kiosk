...
func TestCORSPrefixBypassRejected(t *testing.T) {
	...
	for _, origin := range []string{
		...
		if cfg.isAllowed(origin) {
			t.Errorf("origin %q should be rejected (exact match, not prefix)", origin)
		}
	}
}
...
func TestCORSOptionsPreflight(t *testing.T) {
	...
	if got := rr.Header().Get("Access-Control-Allow-Headers"); got != "Content-Type, Authorization" {
		t.Errorf("expected Allow-Headers to include Authorization, got %q", got)
	}
}
...
func TestCORSDefaultConfigWithBrokerHost(t *testing.T) {
	...
	// Prefix bypass must be rejected for exact-match host entries.
