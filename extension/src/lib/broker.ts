 ...
 *   or malformed body); `kiosks` keeps the last successful listing. Polling
 *   continues so a recovered broker is reported again.
 */
 ...
 *   polls never cause redundant updates.
...
export class BrokerClient {
  ...
  private async pollNow(): Promise<void> {
    ...
    try {
      ...
      if (this.closed) return
      // Network errors, non-OK statuses, and malformed bodies all mean the
      // broker is unreachable right now; keep the last listing and let the
      // recurring poll report recovery.
